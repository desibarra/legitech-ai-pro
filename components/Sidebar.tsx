...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, activeTab, onTabChange }) => {
  ...
  const sidebarClasses = `
    fixed inset-y-0 left-0 z-40 w-72 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out flex flex-col
    ${isOpen ? 'translate-x-0' : '-translate-x-full'}
    md:translate-x-0 md:static md:flex-shrink-0 border-r border-slate-800
  `;
    ...
    <>
      ...
      <aside className={sidebarClasses}>
          ...
          <div className="space-y-3">
             ...
             <div className="grid gap-2">
                {financialIndicators.map((indicator, idx) => (
                    <div key={idx} className="bg-slate-800/40 hover:bg-slate-800 border border-slate-700/50 rounded-lg p-3 transition-colors group">
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-semibold text-slate-400 group-hover:text-indigo-300 transition-colors flex items-center gap-1">
                                {indicator.label}
                            </span>
        ...
        <div className="p-6 bg-slate-900 border-t border-slate-800 shrink-0">
          <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700 backdrop-blur-md">
            <p className="text-xs text-slate-400 uppercase font-bold mb-2">Estado del Sistema</p>
            <div className="flex items-center gap-2">
              <span className="relative flex h-3 w-3">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-500"></span>
              </span>
              <span className="text-xs font-semibold text-emerald-400">Vigilancia Activa</span>
            </div>
          </div>
        </div>
      </aside>
