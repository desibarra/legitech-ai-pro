...
const App: React.FC = () => {
  ...
  const renderTable = () => (
    <div className="bg-slate-800/40 rounded-2xl border border-white/5 shadow-xl overflow-hidden backdrop-blur-sm">
      <div className="px-6 py-5 border-b border-white/5 flex justify-between items-center bg-white/5">
        ...
        <div className="flex gap-2">
          ...
          {activeTab.includes('ISO') && (
            <button className="flex items-center gap-1 text-xs font-bold text-legitech-accent px-3 py-1 rounded-full bg-legitech-accent/10 border border-legitech-accent/20 hover:bg-legitech-accent/20 transition">
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full">
          <thead className="bg-slate-900/50 text-left text-xs uppercase text-slate-400 font-semibold tracking-wider sticky top-0">
            <tr>
            ...
            {displayLaws.map((law) => (
              <tr
                ...
                className={`
                    ...
                    ${selectedLaw?.id === law.id ? 'bg-legitech-primary/20' : 'hover:bg-white/5'}
                `}
              >
                <td className="px-6 py-4 max-w-sm">
                  <div>
                    <p className="font-semibold text-slate-200 text-sm group-hover:text-white transition-colors">{law.title}</p>
                    <p className="text-xs text-slate-500 mt-1 truncate group-hover:text-slate-400">{law.description}</p>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-white/5 text-slate-400 border border-white/10">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${law.impactLevel === 'Alto' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' :
                    law.impactLevel === 'Medio' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                    }`}>
                ...
                <td className="px-6 py-4 text-center">
                  <button className="text-slate-600 group-hover:text-legitech-accent transition">
                    <ArrowRight size={18} />
                  </button>
                </td>
              </tr>
      ...
      <div className="md:hidden p-4 space-y-4">
        {displayLaws.map((law) => (
          <div
            key={law.id}
            onClick={() => handleLawClick(law)}
            className="bg-slate-800/50 rounded-xl p-4 border border-white/5 shadow-sm active:scale-[0.98] transition-transform"
          >
            <div className="flex justify-between items-start mb-3">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase tracking-wide ${law.impactLevel === 'Alto' ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-slate-700/30 border-slate-600/30 text-slate-400'
                }`}>
                {law.impactLevel}
              </span>
              <StatusBadge status={law.status} />
            </div>
            <h4 className="font-bold text-slate-200 text-sm mb-1">{law.title}</h4>
            ...
            <div className="flex justify-between items-center text-xs pt-3 border-t border-white/5">
              <span className="font-medium text-slate-500">{law.isoImpact}</span>
              <span className="text-legitech-accent font-bold flex items-center gap-1">
  ...
  return (
    <div className="flex min-h-screen bg-slate-900 font-sans text-slate-200">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative transition-all duration-300">
        ...
        {/* Modern Header */}
        <header className="h-20 bg-slate-900/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 shrink-0 z-30 sticky top-0">
          <div className="flex items-center gap-4">
            <button
              onClick={() => setSidebarOpen(true)}
              className="md:hidden p-2 hover:bg-white/5 rounded-lg text-slate-400 transition"
            >
              <Menu size={24} />
            </button>
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Smart Search Bar */}
            <div className="hidden md:flex items-center bg-slate-800/50 rounded-full px-4 py-2 w-64 border border-white/5 focus-within:border-legitech-accent/50 focus-within:bg-slate-800 focus-within:ring-2 focus-within:ring-legitech-accent/20 transition-all">
              <Search size={16} className="text-slate-500" />
              <input
                type="text"
                placeholder="Buscar regulación..."
                className="bg-transparent border-none text-sm ml-2 w-full focus:outline-none text-slate-200 placeholder:text-slate-600"
                value={searchQuery}
            ...
            <div className="relative group hidden md:block">
              <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
                <Filter size={14} className="text-legitech-accent" />
              </div>
              <select
                value={industry}
                onChange={(e) => setIndustry(e.target.value as IndustryType)}
                className="pl-9 pr-8 py-2 bg-slate-800 border border-white/10 rounded-lg text-sm font-semibold text-slate-300 focus:ring-2 focus:ring-legitech-accent/50 outline-none cursor-pointer hover:border-legitech-accent/50 transition shadow-sm appearance-none"
              >
            ...
            <button
              ...
              disabled={fetching}
              className="flex items-center gap-2 px-4 py-2 bg-legitech-accent text-slate-900 rounded-lg text-sm font-bold shadow-lg shadow-legitech-accent/20 hover:bg-legitech-accentHover hover:shadow-legitech-accent/30 active:scale-95 transition-all disabled:opacity-70"
            >
            ...
            <button
              ...
              className="p-2 bg-slate-800 border border-white/10 text-slate-400 rounded-lg hover:bg-slate-700 hover:text-white transition shadow-sm"
              title="Subir Evidencia"
            >
              <div className="relative">
                <div className="absolute top-0 right-0 w-2 h-2 bg-rose-500 rounded-full animate-pulse border border-slate-800"></div>
        ...
        {/* Scrollable Content */}
        <main className="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth bg-slate-900">
          <div className="max-w-7xl mx-auto pb-20">
            ...
            {(activeTab === 'Monitor Activo' || activeTab.includes('ISO')) ? (
              <>
                ...
                {activeTab === 'Monitor Activo' ? (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    {[
                      { label: 'Cumplimiento Global', val: '92%', icon: CheckCircle2, color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', trend: '+2.4%' },
                      { label: 'Normas Activas', val: laws.length, icon: TrendingUp, color: 'text-legitech-accent', bg: 'bg-legitech-accent/10', border: 'border-legitech-accent/20', trend: 'Actualizado Hoy' },
                      { label: 'Riesgo Financiero', val: '$8.2M', icon: AlertOctagon, color: 'text-rose-400', bg: 'bg-rose-500/10', border: 'border-rose-500/20', trend: 'Exposición Estimada' },
                    ].map((stat, i) => (
                      <div key={i} className="bg-slate-800/40 p-6 rounded-2xl border border-white/5 shadow-lg hover:bg-slate-800/60 transition-all duration-300 group cursor-default backdrop-blur-sm">
                  ...
                  <div className="mb-8 bg-slate-800/40 rounded-2xl p-6 border border-white/5 shadow-lg backdrop-blur-sm flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                      <div className={`p-4 rounded-2xl ${activeTab.includes('14001') ? 'bg-emerald-500/10 text-emerald-400' : 'bg-blue-500/10 text-blue-400'}`}>
                        {activeTab.includes('14001') ? <Leaf size={32} /> : <ShieldCheck size={32} />}
                      </div>
                      <div>
                        <h2 className="text-2xl font-bold text-white">{activeTab}</h2>
                        <p className="text-slate-400 text-sm">Matriz de requisitos {activeTab.includes('14001') ? 'ambientales' : 'de seguridad y salud'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-8">
                      <div className="text-right">
                        <p className="text-xs text-slate-500 font-bold uppercase">Cumplimiento Norma</p>
                        <p className={`text-3xl font-bold ${complianceStats > 80 ? 'text-emerald-400' : complianceStats > 50 ? 'text-amber-400' : 'text-rose-400'}`}>
                          {complianceStats}%
                        </p>
                      </div>
                      <div className="w-16 h-16 rounded-full border-4 border-slate-700 flex items-center justify-center">
                        <PieChart className="text-legitech-accent" />
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-6">
                <div className="bg-gradient-to-br from-legitech-primary to-legitech-secondary rounded-2xl p-8 text-white shadow-2xl shadow-black/20 border border-white/10">
                  <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <button
                      onClick={() => setModalOpen(true)}
                      className="px-6 py-3 bg-white text-legitech-primary rounded-xl font-bold hover:bg-emerald-50 transition shadow-lg flex items-center gap-2"
                    >
                      <UploadCloud size={20} />
                      Nueva Auditoría
                    </button>
                ...
                <div className="bg-slate-800/40 rounded-2xl border border-white/5 p-8 text-center text-slate-500 backdrop-blur-sm">
                  <div className="flex flex-col items-center gap-3">
                    <FileText size={48} className="text-slate-700" />
                    <p>No hay auditorías recientes en esta sesión.</p>
                    <button onClick={() => setModalOpen(true)} className="text-legitech-accent font-semibold hover:underline">
              ...
              <div className="flex flex-col items-center justify-center py-20 text-slate-500">
                <AlertOctagon size={48} className="mb-4 text-slate-700" />
                <h3 className="text-lg font-bold text-slate-400">Módulo en construcción</h3>
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  }[status] || 'bg-slate-700/30 text-slate-400 border-slate-600/30';
